module.exports = (req, res) => {
    if (req.method === 'POST') {
        const { customer_name, order_number, total_boxes, boxes_in_order, boxNumber } = req.body;

        let zpl = '';

        // Если указана только одна коробка, печатаем её
        if (boxNumber) {
            zpl += generateZPL(customer_name, order_number, boxNumber, total_boxes);
        } 
        // Если указано количество коробок в заказе, печатаем все коробки
        else if (boxes_in_order && total_boxes) {
